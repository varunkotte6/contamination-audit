#!/usr/bin/env python3
"""Phase 2-Qwen — Non-Pythia ground-truth construction.

Addresses reviewer R2's criterion: prove v2 works with a non-Pythia base
family. We fine-tune Qwen2.5-0.5B on the same 5 variants used for
Pythia-1B, to construct an independent ground-truth corpus.

This is a direct replication target. If the Qwen-based v2 ensemble also
produces multi-family Bonferroni-surviving cells on the 22-model audit
matrix, the finding is not Pythia-specific.

Usage: python gt_finetune_qwen.py --variant A --base_short Qwen__Qwen2.5-0.5B
"""
import argparse
import json
import os
...
# Reuse the variant-construction logic from the Pythia script
PROJECT = "{REPO_ROOT}"
LOCALSSD = "/mnt/localssd"
BENCH_DIR = f"{LOCALSSD}/data/benchmarks"

sys.path.insert(0, f"{PROJECT}/code")
# We'll reuse build_variant and MemDataset/PadCollator from gt_finetune
...
def main():
    ...
    ap.add_argument("--base_short", default="Qwen__Qwen2.5-0.5B",
                     help="Subdirectory under /mnt/localssd/models/")
    ...
    metrics = {
        ...
        "verbatim_eval_n": n_eval, "target": 0.8,
        "passed": rate >= 0.8,
    }
