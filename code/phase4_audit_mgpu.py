...
def run_audit(model_id: str, model_path: str):
    ...
    try:
        ...
        return
    # With device_map='auto' inputs must be placed on the first shard.
    try:
        first_device = next(audit_model.parameters()).device
        print(f"[{model_id}] first shard on {first_device}")
